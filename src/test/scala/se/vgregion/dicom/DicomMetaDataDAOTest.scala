...
class DicomMetaDataDAOTest extends FlatSpec with Matchers {
  ...
  val imageFile1 = ImageFile(-1, -1, FileName("file1"))
  val imageFile2 = ImageFile(-1, -1, FileName("file2"))
  val imageFile3 = ImageFile(-1, -1, FileName("file3"))
  val imageFile4 = ImageFile(-1, -1, FileName("file4"))
  val imageFile5 = ImageFile(-1, -1, FileName("file5"))
  val imageFile6 = ImageFile(-1, -1, FileName("file6"))
  val imageFile7 = ImageFile(-1, -1, FileName("file7"))
  val imageFile8 = ImageFile(-1, -1, FileName("file8"))

  val pat1_copy = Patient(-1, PatientName("p1"), PatientID("s1"), PatientBirthDate("1000-01-01"), PatientSex("F"))
  val study3 = Study(-1, -1, StudyInstanceUID("stuid3"), StudyDescription("stdesc3"), StudyDate("19990103"), StudyID("stid3"), AccessionNumber("acc3"))
  ...
  val imageFile9 = ImageFile(-1, -1, FileName("file9"))
  ...
  it should "contain one entry in each table after inserting one patient, study, series, equipent, frame of referenc, image and image file" in {
    db.withSession { implicit session =>
      ...
      dao.insert(imageFile1.copy(imageId = dbImage.id))
  ...
  it should "return the correct number of entities by each category" in {
    db.withSession { implicit session =>
      ...
      dao.insert(imageFile1.copy(imageId = dbImage1.id))
      dao.insert(imageFile2.copy(imageId = dbImage2.id))
      dao.insert(imageFile3.copy(imageId = dbImage3.id))
      dao.insert(imageFile4.copy(imageId = dbImage4.id))
      dao.insert(imageFile5.copy(imageId = dbImage5.id))
      dao.insert(imageFile6.copy(imageId = dbImage6.id))
      dao.insert(imageFile7.copy(imageId = dbImage7.id))
      dao.insert(imageFile8.copy(imageId = dbImage8.id))
      
      dao.patients.size should be (1)
      dao.studiesForPatient(dbPat.id).size should be (2)
      dao.equipments.size should be (3)
      dao.frameOfReferences.size should be (2)
      dao.seriesForStudy(dbStudy1.id).size should be (2)
      dao.seriesForStudy(dbStudy2.id).size should be (2)
      ...
      dao.imageFilesForImage(dbImage1.id).size should be (1)
      dao.imageFilesForImage(dbImage2.id).size should be (1)
      dao.imageFilesForImage(dbImage3.id).size should be (1)
      dao.imageFilesForImage(dbImage4.id).size should be (1)
      dao.imageFilesForImage(dbImage5.id).size should be (1)
      dao.imageFilesForImage(dbImage6.id).size should be (1)
      dao.imageFilesForImage(dbImage7.id).size should be (1)
      dao.imageFilesForImage(dbImage8.id).size should be (1)
