...
class RestTest extends FlatSpec with Matchers with ScalatestRouteTest with RestApi {
  ...
  it should "return an empty list of metadata when monitoring an empty directory and return a list of one metadata entry when a file is copied to that directory" in {
    val tempDir = Files.createTempDirectory("akka-dcm-temp-dir-")
    val tempDirName = tempDir.toString().replace("\\", "/")
    val monitorDir = MonitorDir(tempDirName)
    ...
    Get("/metadata/list") ~> routes ~> check {
      ...
      response count (_ == '{') should be (1) // rough check for one element in list
    }
  }
