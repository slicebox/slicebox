...
class DicomStreamsTest extends TestKit(ActorSystem("AnonymizationFlowSpec")) with AsyncFlatSpecLike with Matchers {
  ...
  "Applying tag modifications when storing DICOM data" should "replace DICOM attributes" in {
    val t1 = TagModification(Tag.PatientName, _ => DicomUtil.padToEvenLength(ByteString("Mapped Patient Name"), VR.PN), insert = true)
    val t2 = TagModification(Tag.PatientID.intValue, _ => DicomUtil.padToEvenLength(ByteString("Mapped Patient ID"), VR.LO), insert = true)
    val t3 = TagModification(Tag.SeriesDescription.intValue, _ => DicomUtil.padToEvenLength(ByteString("Mapped Series Description"), VR.LO), insert = true)
    val bytes = preamble ++ fmiGroupLength(supportedMediaStorageSOPClassUID ++ tsuidExplicitLE) ++ supportedMediaStorageSOPClassUID ++ tsuidExplicitLE ++ patientNameJohnDoe
    val source = Source.single(bytes)
