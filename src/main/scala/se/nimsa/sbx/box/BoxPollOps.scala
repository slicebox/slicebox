...
trait BoxPollOps extends BoxStreamBase with BoxJsonFormats with PlayJsonSupport {
  ...
  def pullBatch(): Future[Seq[OutgoingTransactionImage]] = scaladsl.Source.fromFuture(poll()).runWith(pullSink)

  def poll(): Future[Seq[OutgoingTransactionImage]] =
  ...
  lazy val pollRequest: HttpRequest = {
    val uri = s"${box.baseUrl}/outgoing/poll?n=$batchSize"
