...
class S3Storage(val bucket: String, val s3Prefix: String, val region: String)(implicit system: ActorSystem, materializer: Materializer) extends StorageService {
  ...
  private def credentialsFromProviderChain() = {
    ...
    creds match {
      case sc: AWSSessionCredentials => akka.stream.alpakka.s3.auth.AWSSessionCredentials(sc.getAWSAccessKeyId, sc.getAWSSecretKey, sc.getSessionToken)
      case _ => BasicCredentials(creds.getAWSAccessKeyId, creds.getAWSSecretKey)
    }
  }
