...
The Matlab integration uses the REST API to connect to an instance of slicebox. Several scripts and a simple GUI are provided which can be used to retrieve image information and dicom files. Note that the GUI is created Matlab for OSX, which limits it's usefulness on other OS due to differences in font size etc.
